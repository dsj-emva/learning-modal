...
def wordmark(name: str, tagline: str) -> str:
    ...
    return (f'<div class="k-wordmark"><div class="glyph">{_GLYPH}</div><div><div class="name">{escape(name)}</div>'
            f'<div class="tag">{escape(tagline)}</div></div></div>')


def page_header(eyebrow: str, title: str, lede: str) -> str:
    """Eyebrow, serif title and one muted sentence saying what the page shows."""
    return (f'<div class="k-eyebrow">{escape(eyebrow)}</div><h1 class="k-title">{escape(title)}</h1>'
            f'<p class="k-lede">{escape(lede)}</p>')
...
def section(title: str, text: str = "", step: str | None = None) -> str:
    ...
    return f'<div class="k-section"><div class="h" role="heading" aria-level="3">{s}{escape(title)}</div>{p}</div>'
...
def delta(value: float | None, fmt: str = "pts", good_when: str = "up") -> str:
    ...
    return f'<span class="k-delta {cls}">{escape(text)}</span>'
...
def kpi(label: str, value: str, sub: str = "", delta_html: str = "", unit: str = "", note: str = "") -> str:
    ...
    return (f'<div class="k-kpi"><div class="label">{escape(label)}</div><div class="value">{escape(value)}{u}</div>'
            f'<div class="sub">{delta_html}{escape(sub)}</div>{n}</div>')
...
def stats(items: list[tuple[str, str]]) -> str:
    ...
    cells = "".join(f'<div><div class="label">{escape(k)}</div><div class="v">{escape(v)}</div></div>'
                    for k, v in items)
...
def pill(text: str, kind: str = "info") -> str:
    ...
    return f'<span class="k-pill {escape(kind)}">{escape(text)}</span>'
...
def table(headers: list[str], rows: list[list[str]], numeric_from: int = 1) -> str:
    ...
    def cells(values: list[str], tag: str) -> str:
        return "".join(f'<{tag} class="{"n" if i >= numeric_from else ""}">{escape(v)}</{tag}>'
...
def empty_state(title: str, text: str) -> str:
    ...
    return f'<div class="k-empty"><div class="mark">{escape(title)}</div><p>{escape(text)}</p></div>'


def callout(html_text: str, kind: str = "info") -> str:
    """A tinted note; ``html_text`` must already be escaped (it may carry ``<b>``)."""
    return f'<div class="k-callout {escape(kind)}">{html_text}</div>'
...
def file_card(name: str, label: str, errors: list[Issue], warnings: list[Issue], provided: bool) -> str:
    ...
    for kind, issues in (("error", errors), ("warning", warnings)):
        for i in issues:
            col = f'<span class="col">{escape(i.column)}</span> ' if i.column else ""
            ...
            body += (f'<div class="k-issue {kind}"><div class="bar"></div><div><div class="msg">{col}'
                     f'{escape(i.message)}</div>{rows}</div></div>')
    return (f'<div class="k-file"><div class="head"><div><div class="fname">{escape(name)}</div>'
            f'<div style="font-size:13px;color:var(--muted)">{escape(label)}</div></div>{status}</div>{body}</div>')
...
def result_card(p: float, base_rate: float, deal_value: float, value_at_submit: float, value_formula: float,
    ...
    return (
        ...
        f'<div class="big">{escape(pct(p))}</div>'
        f'<div class="context">Training win rate {escape(pct(base_rate))}{escape(rel)}</div>'
        '<div class="grid">'
        f'<div><div class="label">Expected deal value if won</div><div class="v">{escape(gbp(deal_value))}</div></div>'
        f'<div><div class="label">{escape(submit_label)}</div><div class="v">{escape(gbp(submit_value))}</div></div>'
        '</div>'
        f'<div class="note">{escape(transform)}</div>'
        f'<div class="flags">{flags_html}</div></div>')


__all__ = ["callout", "delta", "empty_state", "file_card", "gbp", "kpi", "kpi_row", "num", "page_header", "pct",
           "pill", "points", "result_card", "section", "stats", "status_pill", "table", "wordmark"]
