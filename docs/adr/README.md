...
| [0020](0020-per-dataset-dates.md) | Each dataset carries its own `as_of` / `test_from` in `dataset.json` (`emva.dataset_meta.dataset_dates`), read by the pipeline and the report; the constants stay the default, so v1/v2 outputs are byte-identical | Proposed | `claude/epic-edison-onl83z` |
| [0021](0021-llm-drafts-mappings.md) | The LLM only drafts a dataset mapping (Haiku 4.5, structured output: target enum + ignore, reason and confidence per column), from redacted column profiles, never rows; cached as ADR 0010; `outcome_confirmed` set by a person, convert refuses otherwise; no derived expressions or dates from the model | Proposed | `claude/epic-edison-onl83z` |
| [0022](0022-results-without-baseline.md) | Results page without a baseline row when the frozen script cannot apply (converted dataset with `dataset.json`, or the script fails): model and status-quo rows, a note instead of the paired comparison, KPI vs status quo else baseline else nothing; store record moves to `keel_meta.json` (decision 6, amends R18) | Proposed | `claude/epic-edison-onl83z` |
| [0023](0023-conversion-rulings.md) | Phase 9 conversion rulings R22-R40 in one place: placeholders, refusals, header-only company/people files, Olist and hotel outcomes, report on converted data, source-format scoring; fix round: baseline omitted only on converted data, optional `won_at`, surfaced derived counts and reorder warnings, status-quo rules in Map & convert | Proposed | `claude/epic-edison-onl83z` |
| [0024](0024-generic-feature-set.md) | Generic feature set: `[[features]]` declared in the mapping and confirmed by a person (`features_confirmed`), raw values in `extra_features.csv`, encoding fitted on the training leads only (levels >= 30 + other + missing; 5 quantile bins + missing) and frozen in the bundle; unseen extra categories are `other` (amends 0009); extras allowed per customer outside the pooled part (amends 0017); pre-registered generic − v2 criterion; (b) format-1 bundles still load, `extra_features.csv` a validated training file, Keel confirms extras separately, offers generic only with extras and shows Generic vs v2; fix round: missingness screen (flag at a closed-vs-open difference >= 0.5), bundle extras iff generic, malformed extras refused, draft names suffixed | Proposed | `phase10-generic-features` |
| [0025](0025-live-draft-check.md) | Live mapping-draft check: `EMVA_ANTHROPIC_API_KEY` alias; the draft reply names `primary_file` and `joins` (amends 0021); prompt guidance then stop, semantic errors left to review; every live reply committed in `mappings/drafts/draft_cache.json` | Proposed | `phase10-generic-features` |
