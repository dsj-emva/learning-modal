...
- **Value transform** (Phase 3, `emva/value_transform.py`): `ValueTransform` = cap at a percentile of the
  ...
  leads, then fixed. Default cap p97 + log + floor £25 (ADR 0012, proposed).
- **value_at_submit / value_at_close** (Phase 3, plan 3.3; horizon `scores.csv` only): `value_at_submit` =
  ...
  `value_at_close` = recorded deal value if Won by AS_OF (blank = 0), at `won_at`; 0 at `matured_at` for a
  mature non-win; NaN while immature (`emva.value.value_at_close`). Upload design: `docs/platform_contract.md`.
