...
adds ``won_within_h``, ``label_source`` and ``matured_at`` to ``scores.csv``. The default
``--feature-set v2`` uses the plan Phase 2 features (see ``emva.features``).
"""
...
def main(argv: list[str] | None = None) -> None:
    ...
    result = run(a.data, margin=a.margin, context=a.context, labels=label_config(ap, a), features=feature_set(a))
